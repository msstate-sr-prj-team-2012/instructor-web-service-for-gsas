...
{
	var html = '<ul id="menu">';
	html += '<li><a href="index.html" class="drop">Home</a></li>';
	html += '<li><a href="data-table.php" class="drop">Table</a></li>';
	html += '<li>Graphs';
	html += '<div class="dropdown_1column align_left">';
	html += '<div class="col_1">';
	html += '<ul class="simple">';
	html += '<li><a href="graph1.php">Shot Vectors</a></li>';
	html += '<li><a href="graph2.php">Bar Graph</a></li>';
	html += '</ul>';
	html += '</div>';
	html += '</div>';
	html += '</li>';
	html += '<li><a href="graph2.php"></a><a href="graph2.php"></a><a href="map.php" class="drop">Map</a></li>';
	html += '</ul>';
...
//Need help converting the rest of the functions function
function makeDivs(rs)
{
	...
	for(var i = 0; i < rs.length); i++)
	{
		...
		str += "<div id="\"tableHoles" +  rh[0]["hole"]["id"] + "div\" class=\"tab2\">";
...
function createShot(rs, cel)
{
	...
	if(rs.length > 0)
	{
		for(var i = 0; i < rs.length; $i++)
		{
			...
			data += "[" + r["id"] + "," + r["holeID"] + ",\"" + r["club"] + "\"," + r["shotNumber"] + "," + distance + "," + angle + "],";
		}
		data = data.substr(0, data.length);
		
		cell = cel + "#tableShots" + s[0]["shot"]["id"] + "div\").show(); history[2]=\"tableShots" + rs[0]["shot"]["id"] + "div\"; }); cell.css(\"cursor\",\"pointer\");";
...
function createHole(rs, cel)
{
	...
	if(rs.length > 0)
	{
		...
		data = data.substr(0, data.length);
		cel += "#tableHoles" + rs[0]["hole"]["id"] + "div\").show();$(\"#larrow\").show(); history[1]=\"tableHoles" + $rs[0]["hole"]["id"] + "div\";});";
		cel += "cell.css(\"cursor\", \"pointer\");";

		str += "$(\"#tableHoles" + rs[0]["hole"]["id"] + "\").dataTable({";
		str += "\"iDisplayLength\": 25,";
		str += "\"aaData\": [" + data + "],";
		str += "\"aoColumns\": [{";
		str += "\"sTitle\": \"Hole Id\"";
...
function createRound(rs)
{
	...
	for(var i = 0; i < rs.length; i++)
	{
		...
		cell += "$($($(\"#tableRounds\").children().eq(1)).children().eq(" + i + ")).children().eq(0);" + "\n" + "cell.click(function(){$(\"#tableRoundsdiv\").hide();$(\"";
